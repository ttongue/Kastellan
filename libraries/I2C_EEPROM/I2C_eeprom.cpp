...
int I2C_eeprom::setBlock(const uint16_t memoryAddress, const uint8_t data, const uint16_t length)
{
    ...
    //rv = _pageBlock(memoryAddress, buffer, length, false);
    return rv;
}
...
int I2C_eeprom::writeBlock(const uint16_t memoryAddress, const uint8_t* buffer, const uint16_t length)
{
    ...
    //rv = _pageBlock(memoryAddress, buffer, length, true);
    return rv;
}
...
bool I2C_eeprom::verifyBlock(const uint16_t memoryAddress, uint8_t* buffer, const uint16_t length)
{
    char inBlox[length];
    memset(inBlox,0,length);
    readBlock(memoryAddress, (uint8_t*)inBlox,length);
    for (int i=0; i< length; i++) {
        if (inBlox[i] != buffer[i]) {
            return false;
        }
    }
    return true;
...
uint16_t I2C_eeprom::generateChecksum(uint8_t* buffer, const uint16_t length)
{
    uint8_t simpleSum=0;
    uint8_t fletcherSum=0;
    for (int i=0; i < length; i++)
    {
        simpleSum=simpleSum+buffer[i];
        fletcherSum=fletcherSum+simpleSum;
    }
    uint16_t outvalue=simpleSum+fletcherSum*256;
    return outvalue;
}
...
uint8_t I2C_eeprom::_ReadBlock(const uint16_t memoryAddress, uint8_t* buffer, const uint8_t length)
{
    ...
    if (rv != 0) 
    {
        //Serial.println("Toot Toot Failtrain.");
        //Serial.flush();
        return 0;  // error
    }
